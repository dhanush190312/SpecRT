...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderResults(data) {
    ...
    document.getElementById('kpiSavedDist').textContent = metrics.distance_saved_mm.toFixed(2);
    document.getElementById('kpiSavedPct').textContent = `${metrics.savings_percent.toFixed(1)}% Reduction`;
    document.getElementById('kpiOptimizedDist').textContent = metrics.optimized_distance_mm.toFixed(2);
    document.getElementById('kpiInitialDist').textContent = metrics.initial_distance_mm.toFixed(2);
    ...
    legs.forEach((leg) => {
      ...
      card.innerHTML = `
        ...
        <span class="step-dist">${leg.distance_mm.toFixed(1)} mm</span>
      `;
  ...
  // =========================================================================
  // Populate Tables (Pure White Typography)
  // =========================================================================
  function populateComparisonTable(data) {
    ...
    const rows = [
      {
        method: 'Initial',
        distance: initial.cost.toFixed(2),
        savings: '0.00 mm (0.0%)',
        route: initial.path_str,
        status: '<span class="badge" style="background: rgba(255,255,255,0.18); color: #FFFFFF;">Baseline</span>',
      },
      {
        method: '2-Opt',
        distance: twoOpt.cost.toFixed(2),
        savings: `${data.metrics.distance_saved_mm.toFixed(2)} mm (${data.metrics.savings_percent.toFixed(1)}%)`,
        route: twoOpt.path_str,
        status: '<span class="badge" style="background: #FFFFFF; color: #961E4C; font-weight: 800;">Optimized</span>',
      },
    ];

    if (heldKarp && heldKarp.cost !== null) {
      const hkSavings = (initial.cost - heldKarp.cost).toFixed(2);
      const hkPct = initial.cost > 0 ? (((initial.cost - heldKarp.cost) / initial.cost) * 100).toFixed(1) : '0.0';
      rows.push({
        method: 'Held-Karp*',
        distance: heldKarp.cost.toFixed(2),
        savings: `${hkSavings} mm (${hkPct}%)`,
        route: heldKarp.path_str,
        status: '<span class="badge" style="background: rgba(255,255,255,0.25); color: #FFFFFF; font-weight: 800;">Exact Global</span>',
      });
    }
    ...
    rows.forEach((r) => {
      ...
      tr.innerHTML = `
        ...
        <td style="font-family: var(--font-mono); font-weight: 700; color: #FFFFFF;">${r.distance}</td>
  ...
  function populateMatrixTable(data) {
    ...
    matrix.forEach((row, i) => {
      ...
      row.forEach((val, j) => {
        ...
        else if (val < 80) cls = 'matrix-low';
        else if (val < 160) cls = 'matrix-med';
        else cls = 'matrix-high';

        rowHtml += `<td class="${cls}">${val.toFixed(1)}</td>`;
      });
      rowHtml += '</tr>';
  ...
  function setupExportHandlers(data) {
    ...
    btnExportCSV.onclick = () => {
      let csv = 'Method,Total Distance (mm),Route\n';
      csv += `Initial,${data.routes.initial.cost},"${data.routes.initial.path_str}"\n`;
      csv += `2-opt,${data.routes.two_opt.cost},"${data.routes.two_opt.path_str}"\n`;
      if (data.routes.held_karp && data.routes.held_karp.cost !== null) {
        csv += `Held-Karp,${data.routes.held_karp.cost},"${data.routes.held_karp.path_str}"\n`;
      }
